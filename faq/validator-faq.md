...
The weight (i.e. voting power) of a validator determines whether or not they are an active validator. Currently, only the top 100 validators with the most voting power will be active validators.&#x20;
...
- Tendermint Key: This is a unique key used to sign consensus votes.
  - It is associated with a public key (Get this value with `provenanced tendermint show-validator`)
  - It is generated when the node is created with `provenanced init`.
- Application key: This key is created from `provenanced` and used to sign transactions. Application keys are associated with a public key prefixed by `pbpub` and an address prefixed by `pb`. Both are derived from account keys generated by `provenanced keys add`.

Note: A validator's operator key is directly tied to an application key, but uses reserved a prefix solely for this purpose: `pbvaloper`.

The testnet prefix is `tp` while the mainnet uses `pb`. These prefixes are switched on the `provenanced` command by including the `-t | --testnet` flag.
