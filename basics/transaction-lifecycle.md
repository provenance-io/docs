...
Next, copy the `address` \(in this example `tp1cuknswnphchtkwe68t4nshcaj4l4azv9ml2qhs`\).

### Testnet Faucet

Provenance testnet provides a Hash faucet that can be used to transfer a small amount of Hash to an address.  We'll use the Faucet in this example to add Hash to our key pair address so we can pay the gas fees required to submit a transaction.

Open the Provenance testnet Hash faucet [https://explorer.test.provenance.io/faucet](https://explorer.test.provenance.io/faucet), paste the value, and press Get Tokens:
...
Now that we have a key pair and have used the testnet Faucet to give our key address gas, we can submit transactions to the blockchain.
